...
100ms高频屏幕捕获 + UIA结构化元素(IUIAutomation) + OCR文字识别 + Delta变化检测

感知层级（第十卷）：
  高频 100ms  → 状态监听、窗口变化、下载进度（纯代码，不用模型）
  中频 500ms  → 卡住判断、页面加载判断、UI状态分析
...
from .screenshot import ScreenCapture, get_screencap
